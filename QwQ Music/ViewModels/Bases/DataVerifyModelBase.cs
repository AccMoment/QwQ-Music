...
public abstract class DataVerifyModelBase : ObservableObject, INotifyDataErrorInfo {
    ...
    public IEnumerable GetErrors(string? propertyName) {
        return string.IsNullOrEmpty(propertyName) || !_errors.TryGetValue(propertyName, out List<string>? errorList) ?
    ...
    protected void ClearAllErrors() {
        List<string> affectedProperties = _errors.Keys.ToList();
        _errors.Clear();
    ...
    protected bool HasErrorFor(string propertyName) {
        return !string.IsNullOrEmpty(propertyName) &&
               _errors.TryGetValue(propertyName, out List<string>? list) &&
               list.Count > 0;
    }
