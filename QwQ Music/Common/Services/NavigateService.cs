...
public static class NavigateService {
    ...
    private static readonly Dictionary<string, string[]?> _viewTree = new() {
        ...
        { "歌单", ["全部歌单", "歌单详情"] },
        { "专辑", ["全部专辑", "专辑详情"] },
        { "音效", ["立体增强", "全景效果", "空间效果", "环绕效果", "混响效果", "延迟效果", "失真效果", "颤音效果", "压缩器", "均衡器", "噪音减少"] }
    };
    ...
    static NavigateService() { _navigationHistory.Add(CurrentView); }
        ...
        do {
            ...
            for (int block = 1; block <= n / 2; block++) {
                ...
                for (int k = 0; k < block; k++) {
                    if (_navigationHistory[n - 1 - k] == _navigationHistory[n - 1 - block - k])
                        continue;
    ...
    public static void NavigateEvent(string viewName, int targetChildIndex) {
        if (!_viewTree.TryGetValue(viewName, out string[]? children) ||
            children == null ||
            targetChildIndex < 0 ||
        ...
        if (ViewIndex.TryGetValue(viewName, out (string? Parent, int IndexInParent, int CurrentChildIndex) indexInfo))
        ...
        while (_viewTree.TryGetValue(deepestView, out string[]? grandChildren) &&
               grandChildren != null &&
               ViewIndex.TryGetValue(
                   deepestView,
                   out (string? Parent, int IndexInParent, int CurrentChildIndex) childIndexInfo) &&
        ...
        ComeToOneselfEvents.TryGetValue(CurrentView, out Action? navigateAction);
    ...
    public static void NavigateTo(string targetViewName) {
        if (!ViewIndex.TryGetValue(
                targetViewName,
                out (string? Parent, int IndexInParent, int CurrentChildIndex) targetIndexInfo))
            return; // 目标视图不存在
        ...
        while (currentParentName != null &&
               ViewIndex.TryGetValue(
                   currentParentName,
                   out (string? Parent, int IndexInParent, int CurrentChildIndex) parentIndexInfo)) {
        ...
        while (navigationStack.Count > 0) {
            ...
            NavigateToEvents.TryGetValue(parentName, out Action<int>? action);
    ...
    public static string? GetParentView(string viewName) {
        return ViewIndex.TryGetValue(
            viewName,
            out (string? Parent, int IndexInParent, int CurrentChildIndex) indexInfo) ?
            indexInfo.Parent :
            null;
    }
