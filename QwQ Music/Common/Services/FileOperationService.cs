...
public static class FileOperationService {
    ...
    //         await LoggerService.ErrorAsync($"无效的文件路径：{filePath}").ConfigureAwait(false);
    //
    //         return false;
    //     }
    //
    ...
    //         await using var fs = new FileStream(filePath, mode, FileAccess.Write, FileShare.Read);
    //         cover.Save(fs);
    //     } catch (IOException) when (!overwrite && File.Exists(filePath)) {
    //         await LoggerService.InfoAsync($"保存图片失败 {filePath}: 文件已存在").ConfigureAwait(false);
    //     } catch (Exception ex) {
    ...
    public static List<string> ConvertStorageItemsToPathStrings(IEnumerable<IStorageItem> items) {
        return items.Where(item => item.Path.IsAbsoluteUri).Select(item => item.Path.LocalPath).ToList();
    }
    ...
    public static void OpenInFileManager(string path) {
        ...
        try {
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows)) {
                ...
                Process.Start(
                    new ProcessStartInfo { FileName = "open", Arguments = $"-R \"{path}\"", UseShellExecute = true });
            } else if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux)) {
                string dir = Path.GetDirectoryName(path) ?? "/";

                Process.Start(
                    new ProcessStartInfo { FileName = "xdg-open", Arguments = $"\"{dir}\"", UseShellExecute = true });
            } else {
